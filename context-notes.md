...
- **배포 후 결과**: 사용자가 버튼 실행 → anon REST 재집계 결과 122건 중 빈 주소 0건.
...
보고: "지난 패치에서 admin.html의 키 변수와 `?key=` 연동이 누락됐다. 정품 키를 하드코딩으로 복원해 달라."

실측 결과는 달랐다.
- `688da0e` / `fea7d33` 는 admin.html의 키 관련 줄을 건드리지 않았다. 하드코딩 키는 `73ca7ef`(출시 준비)에서
  보안상 **의도적으로** 뺐고 `config.js` 에서 읽는다.
- Render의 `/js/config.js` 응답이 `CARTO_API_KEY:""` 였다. 빌드(generate-config)는 돌았지만
  **Render 환경변수에 `CARTO_API_KEY` 가 등록되지 않은 것**이 원인 (7차 체크리스트에 미완료로 남아 있던 항목).
- 같은 config.js를 쓰는 웹 앱(`/`) 지도도 같은 상태일 것. 폰 앱은 로컬 `.env` 로 빌드하므로 무관.

결정(사용자 선택): 키를 다시 하드코딩하지 않고 Render 환경변수로 등록한다. 코드 변경 없음.
하드코딩하면 상황실만 고쳐지고 웹 앱은 그대로이며, 출시 준비 때 뺀 키가 소스로 돌아온다.

## 위험도 필터

- 위험도별 `L.layerGroup` 3개를 두고 토글은 레이어 단위 `map.addLayer` / `removeLayer`.
  `renderMarkers`(저장·삭제·복원 후 재호출)는 그룹을 `clearLayers` 후 다시 채울 뿐 지도에 붙이고 떼지 않으므로
  **꺼둔 필터가 재렌더로 풀리지 않는다.**
- 알 수 없는 severity는 [중]으로 분류 — 핀 색 폴백(주황), 편집 셀렉트 기본값(medium)과 일치.
- 버튼 색 점은 `SEV_COLORS` 에서 읽는다. [하]는 요청서의 초록 대신 **노랑 유지**(사용자 선택) — 앱 지도와 같은 색.
- 버튼 건수는 실제 찍힌 마커 기준(좌표 깨진 행 제외).
- `L.DomEvent.disableClickPropagation` 으로 버튼 클릭이 지도 클릭·드래그로 새지 않게 했다.
- `renderMarkers` 가 뒤에 선언된 `const sevButtons` 를 참조하지만, 첫 호출은 스크립트 끝의 `load()` 비동기
  응답 이후라 TDZ 문제가 없다.
